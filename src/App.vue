...
const Generate = () => {
  if (data.anggota > 0 && (data.kelompok > 0 && data.kelompok <= data.anggota)) {
    ...
    console.log(DataGenerate.Kelompok);
  }
}
...
<template>
  ...
  <div class="bg-slate-100 w-full h-4/5 m-4 flex flex-wrap justify-center overflow-y-scroll rounded">
    <!-- CARD -->
    <div class="bg-slate-100 w-80 h-1/2 flex m-2 flex-col overflow-y-scroll rounded shadow-md" v-for="(item, index) in DataGenerate.Kelompok" :key="index">
      ...
      <div v-for="(id, no) in item" class="w-full flex flex-wrap shadow-md z-10" :class="[no % 2 == 0 ? 'z-0' : 'z-10']">
        ...
        <input class="h-7 w-4/5 text-center outline-none truncate px-2" :class="[no % 2 == 0 ? 'bg-slate-100' : 'bg-slate-200']" placeholder="Nama Anggota...">
      </div>
    </div>
  </div>
  <!-- Copyright -->
